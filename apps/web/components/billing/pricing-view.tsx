...
export function PricingView() {
  ...
  async function upgrade(planCode: string) {
    ...
    setError('');
    try {
      const checkout = await startProCheckout(accessToken, discountCode, planCode);
  ...
  return (
    <div className="grid gap-8">
      <header className="grid gap-4 text-center">
        ...
        <h1 className="text-4xl font-semibold tracking-[-0.07em] sm:text-6xl">All the essentials. More capacity when you need it.</h1>
        <p className="mx-auto max-w-2xl text-base leading-8 text-brand-mid">Every plan includes Vrompt&apos;s core features. Paid plans add significantly higher configurable usage limits.</p>
      </header>
      {error ? <p className="text-center text-sm text-red-600 dark:text-red-300" role="alert">{error}</p> : null}
      {user ? (
        <div className="mx-auto grid w-full max-w-xl gap-2 rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
          <label className="text-sm font-semibold" htmlFor="discount-code">Promotion code (optional)</label>
          <input className="min-h-11 rounded-xl border border-zinc-300 bg-transparent px-3 text-sm uppercase dark:border-zinc-700" id="discount-code" maxLength={64} onChange={(event) => setDiscountCode(event.target.value)} placeholder="Enter a code" value={discountCode} />
          <p className="text-xs text-brand-mid">The backend validates eligibility and calculates the final checkout price.</p>
        </div>
      ) : null}
      {!plans ? (
        <div className="grid gap-4 md:grid-cols-2"><Skeleton className="h-96" /><Skeleton className="h-96" /></div>
      ) : (
        <div className={`grid gap-4 ${plans.length > 2 ? 'lg:grid-cols-3' : 'md:grid-cols-2'}`}>
          {plans.map((plan) => {
            const isCurrent = user?.plan === plan.id;
            const isPaid = plan.originalPrice > 0;
            const remainingMs = plan.promotion ? Math.max(new Date(plan.promotion.endsAt).getTime() - now, 0) : 0;
            return (
              <Card className={isPaid ? 'border-brand-mid' : ''} key={plan.id}>
                ...
                <div className="mt-8">
                  {isCurrent ? <Link className={getButtonClasses('secondary') + ' w-full'} href={'/billing' as Route}>View billing</Link> : isPaid ? user ? <Button className="w-full" disabled={Boolean(busyPlan)} onClick={() => void upgrade(plan.id)}>{busyPlan === plan.id ? 'Opening secure checkout…' : `Choose ${plan.name}`}</Button> : <AuthModalTrigger className={getButtonClasses('primary') + ' w-full'} description="Sign in so Vrompt can securely attach your purchase to your account." returnTo="/pricing" title={`Choose ${plan.name}`}>Sign in to upgrade</AuthModalTrigger> : null}
                </div>
                {plan.promotion ? <div className="mt-5 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900 dark:bg-amber-950/40 dark:text-amber-200"><strong>{plan.promotion.name}</strong> · Ends {new Date(plan.promotion.endsAt).toLocaleString(undefined, { timeZone: plan.promotion.timezone })} ({plan.promotion.timezone}). {remainingMs > 0 ? `${Math.floor(remainingMs / 3_600_000)}h ${Math.floor((remainingMs % 3_600_000) / 60_000)}m remaining.` : 'Offer expired.'}</div> : null}
              </Card>
            );
          })}
        </div>
